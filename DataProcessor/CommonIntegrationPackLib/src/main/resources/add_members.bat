@echo off
	...
	echo "Add members"
	type add_members.conf

	%EMS_HOME%\bin\tibemsadmin -script add_members.conf -server tcp://%1:%2 -user %3 -password %4
...
:ERRORHANDLING
echo ERROR: Failed to invoked tibemsadmin to run add_members.conf.. RC=%ERR_LVL%
