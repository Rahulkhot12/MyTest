@echo off
	...
	echo "Create Topics"
	type create_topics.conf

	%EMS_HOME%\bin\tibemsadmin -script create_topics.conf -server tcp://%1:%2 -user %3 -password %4
...
:ERRORHANDLING
echo ERROR: Failed to invoked tibemsadmin to run create_topics.conf.. RC=%ERR_LVL%
