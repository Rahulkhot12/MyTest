@echo off
	...
	echo "Create factories"
	type create_factories.conf

	%EMS_HOME%\bin\tibemsadmin -script create_factories.conf -server tcp://%1:%2 -user %3 -password %4
...
:ERRORHANDLING
echo ERROR: Failed to invoked tibemsadmin to run create_factories.conf.. RC=%ERR_LVL%
