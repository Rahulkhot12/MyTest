@echo off
	...
	echo "Create groups"
	type create_groups.conf

	%EMS_HOME%\bin\tibemsadmin -script create_groups.conf -server tcp://%1:%2 -user %3 -password %4
...
:ERRORHANDLING
echo ERROR: Failed to invoked tibemsadmin to run create_groups.conf.. RC=%ERR_LVL%
