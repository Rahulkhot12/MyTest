@echo off
	...
	echo "Create Bridge"
	type create_bridges.conf

	%EMS_HOME%\bin\tibemsadmin -script create_bridges.conf -server tcp://%1:%2 -user %3 -password %4
...
:ERRORHANDLING
echo ERROR: Failed to invoked tibemsadmin to run create_bridges.conf.. RC=%ERR_LVL%
