@echo off
	...
	echo "Grant queues"
	type grant_queues.conf

	%EMS_HOME%\bin\tibemsadmin -script grant_queues.conf -server tcp://%1:%2 -user %3 -password %4
...
:ERRORHANDLING
echo ERROR: Failed to invoked tibemsadmin to run grant_queues.conf.. RC=%ERR_LVL%
