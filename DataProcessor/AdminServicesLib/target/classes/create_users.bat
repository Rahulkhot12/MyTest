@echo off
	...
	echo "Create users"
	type create_users.conf

	%EMS_HOME%\bin\tibemsadmin -script create_users.conf -server tcp://%1:%2 -user %3 -password %4
...
:ERRORHANDLING
echo ERROR: Failed to invoked tibemsadmin to run create_users.conf.. RC=%ERR_LVL%
