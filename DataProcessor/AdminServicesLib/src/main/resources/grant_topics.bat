@echo off
	...
	echo "Grant topics"
	type grant_topics.conf

	%EMS_HOME%\bin\tibemsadmin -script grant_topics.conf -server tcp://%1:%2 -user %3 -password %4
...
:ERRORHANDLING
echo ERROR: Failed to invoked tibemsadmin to run grant_topics.conf.. RC=%ERR_LVL%
